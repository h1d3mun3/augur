...
# These four are assigned in the dispatch tail the AUGUR_SOURCE_ONLY seam returns before, so mint
# them here exactly as that tail does — cmd_up's messages interpolate CONTAINER_NAME and augur runs
# under `set -u`.
