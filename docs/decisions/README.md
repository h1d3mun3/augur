...
| [0016](./0016-shared-file-cache-refresh.md) | ~~Refresh the guest's view of shared files with `msync(MS_INVALIDATE)`~~ — **superseded by 0017.** The host computed which shared files had changed since the last sweep (`find -newer`) and the guest invalidated exactly those with `msync(addr, len, MS_INVALIDATE)` on a `PROT_READ` mapping, at every attach point plus a 5 s host-side loop, with a boot-time self-test that reported both when the mitigation broke and when the platform no longer needed it. Removed from `main`; preserved on `topic/virtiofs-cache`. Kept for the historical rationale and for the 105-arm measurements in §1, which 0017 accepts the defect on. |
| [0017](./0017-accept-virtiofs-staleness.md) | **The macOS virtiofs staleness is accepted, not mitigated** — reverses 0016 in full: the sweep, the refresher loop, the freshness self-test, the `--share-refresh` dials, `augur refresh --macos`, the `augur config` settings layer that existed only to persist them, and the older `warn_if_macos_profile_stale` are all removed. The defect (#124/#135, both still open) is unchanged and is Apple's to fix — no timeout to wait out (904.9 s measured, cleared only by guest vnode reclaim), not specific to read-only shares, and with `stat` disagreeing with `read` so no guest-side freshness check can be right. Withdrawn because the mitigation bought *bounded staleness* rather than coherence, never reached directory entries at all, and kept generating its own silent-failure defects (four found in nine days, including a refresher that reported healthy for 31 hours while invalidating nothing). The remedy is `down --macos && up --macos`, which works because the guest reboots with an empty vnode cache — not because the share device is rebuilt. |
| [0018](./0018-macos27-unattended-provisioning.md) | **macOS 27's `VZMacGuestProvisioningOptions` replaces the manual Setup Assistant step, and the base VM's admin password is randomized on that path** — on a macOS 27+ host installing a macOS 27+ guest, `cmd_build_macos` provisions the admin account headlessly (no GUI, no operator-typed credential, Remote Login enabled by the framework) with a randomly generated, host-persisted password (`~/.augur/macos-admin-password`). This is exactly the "out of scope" future ADR-0007 named: randomizing is now safe because the password is set *before* the account exists, so there's no keychain-desync footgun to avoid. Every other combination (older host, older guest IPSW, or a pre-existing base VM) takes ADR-0007's original manual flow unchanged, with the fixed `admin`/`admin` credential. |

> Numbering: 0001 is the foundational principle (recorded latest but conceptually first);
> 0002–0018 follow their original decision dates.
