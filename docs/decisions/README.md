...
| [0012](./0012-drop-workspace-trust-seed.md) | **augur no longer pre-trusts the mounted workspace** — reverses 0011 in full: Claude Code's own folder-trust dialog now runs in the guest like anywhere else, closing the silent, unreviewed auto-approval of a repo's `permissions.allow`/`hooks` that pre-trust granted. The lost `/agents`-after-`destroy` convenience was a recoverable one-time prompt, not a real cost. Also fixes a macOS-mode bug the same change exposed: `cmd_up_macos` was overwriting `~/.claude.json` unconditionally on every `up`, clobbering accumulated guest state once VM clones started persisting across `down` (0006/0010); the fix writes the now-generic onboarding stub only when this `up` just **cloned** the VM — gated on the clone, deliberately *not* on the file's absence, which 0012 records as a weaker check masquerading as the same thing: the base VM is a long-lived mutable disk that can accumulate its own stale `.claude.json` between builds, so an existence check silently inherits it into every new project clone. |
