# Design: per-run agent/LLM profiles (local Ollama, alternate agents)

- **Date:** 2026-06-27
- **Scope:** Letting `augur` run a coding agent against a non-Anthropic LLM endpoint — primarily a LAN or Tailscale [Ollama](https://ollama.com) server addressed by IP — for a **single run**, selected at launch, in both Docker and macOS VM modes, **without weakening the egress guarantee**; and generalizing the mechanism so the agent itself (Claude Code, Aider, …) is a swappable payload rather than a hard dependency.
- **Status:** Living document — re-verify when `cmd_claude` / `cmd_claude_macos`, `write_merged_allowlist` / `strip_project_ip_rules`, `resolve_llm_profile`, the arg parser, or `Allowlist`/`Filter`/`SocketIO` change.
