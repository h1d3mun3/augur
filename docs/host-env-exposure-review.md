...
- **macOS** shares `~/.config/gh` (`:ro`) and the workspace via virtiofs (`--dir`), keeps per-VM Claude history under `~/.augur/claude-projects/<vm>`, and scp's `~/.gitconfig` (in `cmd_up_macos`). `~/.claude` itself is **not** shared.
