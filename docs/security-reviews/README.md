...
| Date | Scope | File |
|---|---|---|
| 2026-09-24 | Current-state review focused on egress control (re-baseline — PR #212/[ADR-0021](../decisions/0021-container-per-session-credentials.md) moves Apple Container credentials off `container run` onto a per-session `--env-file` pipe written by builtin `printf`, closing M1 (item 6) apart from bundles left by an older augur, and adds a 1.0.0 Container CLI floor; `cmd_up` split into reconcile / reuse / recreate (#201) with the containment argv diffed unchanged; gvproxy re-pinned `b6bb07ed`→`fc319b8f` (closes #186, test-only rebase, DNS gate still first); `augur-proxy/`'s tree hash moves for the first time since 2026-07-28, comment-only. **Contract change** — I10 goes ⚠ review-only → 🟡 partial (Container half test-enforced) and records the macOS admin password as its accepted residual. **New residuals, items 46/47:** a persisted guest can spoof the reuse-path self-test and the setup-token integrity gate on both engines) | [2026-09-24-egress.md](./2026-09-24-egress.md) |
| 2026-09-21 | Current-state review focused on egress control (re-baseline — no engine change; `augur-proxy/` and `augur.conf` byte-identical to 2026-09-05 for a fourth consecutive snapshot. gvproxy re-pinned twice more, `d3d4f055`→`50edb65e` (closes #172, dependency-only)→`b6bb07ed` (closes #175, a hand-rebased conflict with upstream's new SOA/PTR/AAAA DNS-forwarding feature, allowlist gate confirmed to still run first); ADR-0018 adds unattended macOS 27+ base-VM admin provisioning (generated password kept off argv, pushed via stdin) with a same-day-fixed build-time argv leak (`933f0ee`); host-timezone injection lands on both guest kinds (PR #182). **New residual, item 44:** the five ongoing `sudo -S` call sites against a running macOS clone still embed the (now-real, ADR-0018-generated) admin password in local `ssh` argv, unlike the build-time path's stdin fix. **Not a contract change** — `INVARIANTS.md` is byte-untouched) | [2026-09-21-egress.md](./2026-09-21-egress.md) |
| 2026-09-05 | Current-state review focused on egress control (re-baseline — PR #168/[ADR-0017](../decisions/0017-accept-virtiofs-staleness.md) withdraws the entire #124/#135 virtiofs-staleness mitigation merged as PR #152 (share-refresh sweep, guest `msync` invalidator, freshness tripwire, `--share-refresh` dial, tests `41`–`43`), accepting the underlying platform defect instead of patching it; the `gh-config` share stays removed via a standalone same-day fix; gvproxy re-pinned `629a4a42`→`d3d4f055` (closes #167), with a same-day fix threading `connectTimeout` into the SOCKS dial path a rebase had left unbounded. **Not a contract change** — `INVARIANTS.md` is byte-untouched) | [2026-09-05-egress.md](./2026-09-05-egress.md) |
